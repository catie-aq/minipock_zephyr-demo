...
document.getElementById('checkUpdateButton').addEventListener('click', function() {
    ...
    updateStatus.innerHTML = '<span class="badge bg-info">Checking for updates...</span>';
    ...
    setTimeout(() => {
        ...
        if (latestVersion > currentVersion) {
            ...
            checkButton.innerHTML = '<i class="bi bi-github me-2"></i>Update from GitHub';
            checkButton.classList.remove('btn-info');
            checkButton.classList.add('btn-warning');
        } else {
            // No update needed
            updateStatus.innerHTML = '<span class="badge bg-success">System is up to date</span>';
            checkButton.innerHTML = '<i class="bi bi-search me-2"></i>Check for Updates';
